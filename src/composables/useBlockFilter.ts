...
export function passesBlockFilter(
  comic: ComicBrief,
  blockedTags: string[],
  blockedNames: string[],
...
/** 响应式过滤列表。 */
export function useBlockFilter(items: Ref<ComicBrief[]>) {
  ...
  return computed(() => {
    ...
    return items.value.filter((c) =>
      passesBlockFilter(c, blockedTags, blockedNames, blockedAuthors),
    )
  })
}
